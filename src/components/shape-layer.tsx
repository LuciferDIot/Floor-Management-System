...
export default function ShapeLayer({ floorId }: ShapeLayerProps) {
  ...
  const handleSelect = (e: KonvaEventObject<Event>, shape: Shape) => {
    const isShift = (e.evt as KeyboardEvent).shiftKey;
  ...
  const handleTransformEnd = (shape: Shape) => {
    ...
    const transformedNode = node.nodes()[0];
  ...
  const selectedShapeObjects =
    floor?.items.filter((item) => selectedShapes.includes(item.id)) || [];

  if (!floor) return null;

  return (
    <>
      {floor.items.map((shape) => {
        const isSelected = selectedShapes.includes(shape.id);

        // Store ref for the shape
        const setRef = (ref: Konva.Node | null) => {
          if (ref) {
